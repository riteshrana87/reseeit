    ...
    <div class = "container-fluid">
        ...
        <div class = "row header-reg headerPanel-nvt">
            ...
            <div class = "col-sm-12 col-md-6 col-lg-5">
                <p class="col-sm-5 col-md-12 plat-greet" style="display: inline;" id="client-greeting">
                    Hello <?php
                    if(!empty($userlist[0]['first_name'])){
                        echo $userlist[0]['first_name'];
                     }else{
                        echo $user_session['first_name'];
                    }?>
                    </p>
        ...
        <div class="row outer1 display-yes" id="loyaltyInteractions">
            <div class="mainBody col-sm-12 col-lg-10 col-lg-offset-1" style="color: black; text-align: left; font-size:0.8em;height: 615px;">
                <?php if(empty($userlist[0]['com_id'])) { ?>
                <form id="form-inAppAds" method="post"
                      action="<?php echo base_url(); ?>home/location_admin_account/insert_data" ENCTYPE="multipart/form-data" onsubmit="return confirmPass();">
                    <?php }else{?>
                    <form id="form-inAppAds" method="post"
                          action="<?php echo base_url(); ?>home/location_admin_account/update_data" ENCTYPE="multipart/form-data" onsubmit="return confirmPass();">
                        <?php }?>
                        ...
                        <div class = "col-xs-12 col-sm-6">
                            <div class="right-head" style="margin: 5px 0px 5px 0px">
                                ...
                                <div class="form-group">
                                    <label for="locBizName">Name of Business</label>
                                ...
                                <div class="form-group marginTop3">
                                    <label style="width: 100%" for="locBizAbout">Description</label>
                                    <div class="row">
                                        <div class = "col-xs-12">
                                            <textarea required class="form-control" name="description" rows = "2" name="title" id="locBizAbout" placeholder="About the Business (150 characters max.)"><?=!empty($userlist[0]['description'])?$userlist[0]['description']:''?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="locStreetAddress">Address</label>
                                    ...
                                    <?php if($userlist){?>
                                        <input type="hidden" name="com_id" value="<?php echo $userlist[0]['com_id']?>" required>
                                        <input type="hidden" name="com_location_id" value="<?php echo $userlist[0]['com_location_id']?>" required>
                                    <?php }?>
                                    <div class="row">
                                        ...
                                        <div class="col-xs-6 col-sm-3 marginTop3">
                                            <input type="text" value="<?=!empty($userlist[0]['city'])?$userlist[0]['city']:''?>" name="city" class="form-control" id="clientZip" placeholder="City" >
                                        </div>
                                        <div class="col-xs-6 col-sm-3 marginTop3">
                                            <input type="number" value="<?=!empty($userlist[0]['zipcode'])?$userlist[0]['zipcode']:''?>" name="zipcode" class="form-control" id="clientZip" placeholder="Zip Code" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label style="width: 100%" for="locContactFirstName">Location Admin Information</label>
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-6 marginTop3">
                                            <input type="text" name="first_name" value="<?=!empty($userlist[0]['first_name'])?$userlist[0]['first_name']:''?>" class="form-control" id="clientContactFirstName" placeholder="First Name" required>
                                        </div>
                                        <div class="col-xs-12 col-sm-6 marginTop3">
                                            <input type="text" name="last_name" value="<?=!empty($userlist[0]['last_name'])?$userlist[0]['last_name']:''?>" class="form-control" id="clientContactLastName" placeholder="Last Name" required>
                                        </div>
                                    </div>
...
<script type="text/javascript">
    function check_email(emailData,userid)
    {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url().'home/location_admin_account/check_user';?>",
            //dataType: 'json',
            async: false,
