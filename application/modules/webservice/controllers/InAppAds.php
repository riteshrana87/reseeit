...
class InAppAds extends REST_Controller {
...

    {
        ...
        $app_where = "( status =1)";
        $app_add_info = $this -> Common_model -> retrieve($this->app_add, $app_where);
        foreach($app_add_info as $app_add_data){
            if(!empty($app_add_data->app_img)){
                $app_add_info_data[]=$app_add_data;
            }
        }
//pr($app_add_info_data);exit;
