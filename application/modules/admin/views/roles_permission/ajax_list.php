...
<table class="table table-bordered table-striped dataTable" id="example1" role="grid" aria-describedby="example1_info">
    <thead>
        ...
        <th <?php if(isset($sortfield) && $sortfield == 'role_name'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}}else{echo "class = 'sorting'";} ?> tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 10%;" onclick="apply_sorting('role_name','<?php echo $sorttypepass;?>')"><?=$this->lang->line('roles_label_name')?></th>
            ...
            <th <?php if(isset($sortfield) && $sortfield == 'role_pname'){if($sortby == 'asc'){echo "class = 'sorting_desc'";}else{echo "class = 'sorting_asc'";}}else{echo "class = 'sorting'";} ?> tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 10%;" onclick="apply_sorting('role_pname','<?php echo $sorttypepass;?>')"><?=$this->lang->line('roles_permission_label_name')?></th>
    ...
    <?php if(!empty($datalist)) {
      ...
      foreach($datalist as $row) { 
        ...
        <tr role="row" class="odd">
            ...
            <td><?=!empty($row['role_name'])?$row['role_pname']:''?></td>
            <td class="hidden-xs hidden-sm">

                <a class="btn btn-xs btn-primary" href="<?= $this->config->item('admin_base_url').$this->viewname; ?>/edit_record/<?= $row['role_pid'] ?>" title="Edit Record"><i class="fa fa-pencil"></i></a> &nbsp;
                <button class="btn btn-xs btn-danger" title="Delete Record"  onclick="deletepopup('<?php echo  $row['role_pid'] ?>','<?php echo $name; ?>');"> <i class="fa fa-times"></i> </button>
            </td>
          </tr>
        <?php } ?> 
        <tfoot>
        <tr>
            ...
            <th rowspan="1" colspan="1"><?=$this->lang->line('roles_permission_label_name')?></th>
