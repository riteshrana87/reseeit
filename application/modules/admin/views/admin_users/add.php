                ...
                <form id="<?=$this->viewname?>" data-parsley-validate method="post" action="<?=base_url($path)?>"  ENCTYPE="multipart/form-data">
                  <div class="box-body">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" name="name" id="name" value="<?=!empty($editRecord[0]['fullname'])?$editRecord[0]['fullname']:''?>" class="form-control" required >
                    </div>
                    <div class="form-group">
                      <label for="email"><?=$this->lang->line('common_label_email')?>:</label>
                      <input type="email" name="email" id="email" value="<?=!empty($editRecord[0]['email'])?$editRecord[0]['email']:''?>" class="form-control" data-parsley-trigger="change" onblur="return check_email(this.value,'<?=!empty($editRecord[0]['user_id'])?$editRecord[0]['user_id']:''?>');" required>
                    </div>
                   ...
                   <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <input type="password" name="cpassword" placeholder="confirm password" id="cpassword" class="form-control parsley-validated" type="text" required data-parsley-equalto="#password" />                   
                    </div>
                    <?php } ?>
                    <div class="form-group">
                        ...
                        <?php if(empty($editRecord)) { ?>
                         <img id="uploadPreview1" class="noimage" src="<?=$this->config->item('admin_image_path').'no_image.jpg'?>"  width="100" />
                          <?php } else { ?>
                                ...
                                <?php } else { 
                                  //echo "em";
                                  if(!file_exists($this->config->item('admin_user_small_img_upload_path').$editRecord[0]['user_img'])){
                                    ?>
                                     <img id="uploadPreview1" class="noimage" src="<?=$this->config->item('admin_image_path').'no_image.jpg'?>"  width="100" />
                                <?php }
                                else
                                { 
                                  ?>
                                  <img id="uploadPreview1" src="<?=$this->config->item('admin_user_small_img_url').$editRecord[0]['user_img'] ?>"  width="100"  height="100" />
