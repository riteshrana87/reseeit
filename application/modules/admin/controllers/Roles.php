...
class Roles extends CI_Controller
{
	...
	*/
	...
	public function index()
	{
		...
		if(!empty($allflag) && ($allflag == 'all' || $allflag == 'changesorting' || $allflag == 'changesearch')) {
			$this->session->unset_userdata('roles_sortsearchpage_data');
		}

		$searchsort_session = $this->session->userdata('roles_sortsearchpage_data');
		//Sorting
		...
		//Query
		$table            = ROLES_TABLE.' as u';
		$fields           = array('u.*');

		if(!empty($searchtext))
		{
			$searchtext = html_entity_decode(trim($searchtext));
			$match=array('u.role_name'=>$searchtext);
		...
		$this->session->set_userdata('roles_sortsearchpage_data', $sortsearchpage_data);
    ...
    */
	...
	{
		$role_data['role_name']        = trim(ucfirst($this->input->post('role_name')));
		$role_data['role_level']          = $this->input->post('role_level');

		$role_id = $this->general_model->insert(ROLES_TABLE,$role_data);
    ...
    */
	public function update_data()
	{
		...
		$role_data['role_name']        = trim(ucfirst($this->input->post('role_name')));
		$role_data['role_level']             = trim($this->input->post('role_level'));
		$where = array('role_id' => $id);
		$role_id = $this->general_model->update(ROLES_TABLE, $role_data, $where);
    ...
    */
	public function ajax_delete_all()
	{
		...
		{
			$where = array('role_id' => $id);
			$this->general_model->delete(ROLES_TABLE,$where);
			unset($id);
		}
